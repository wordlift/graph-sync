...
code=$(run_case install_python3 "${BASE_ENV[@]}" INPUT_WORAI_VERSION="6.9.4" "$SCRIPT")
...
assert_eq "-m pip install worai==6.9.4" "$second_call" "second command installs pinned worai"

# Case 4: missing python interpreters
EMPTY_BIN="$TMP_DIR/empty-bin"
mkdir -p "$EMPTY_BIN"
code=$(run_case no_python /usr/bin/env PATH="$EMPTY_BIN" INPUT_WORAI_VERSION="6.9.4" /bin/bash "$SCRIPT")
