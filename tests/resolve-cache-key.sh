...
code=$(run_case fallback env GITHUB_OUTPUT="$out_file" INPUT_CACHE_ENABLED="yes" INPUT_WORAI_VERSION="6.0.0" INPUT_PLAYWRIGHT_VERSION="1.55.0" INPUT_PLAYWRIGHT_BROWSER="chromium" "$SCRIPT")
assert_eq "0" "$code" "fallback suffix should succeed"
suffix=$(read_output_value "cache_key_suffix" "$out_file")
assert_eq "6.0.0-1.55.0-chromium" "$suffix" "fallback suffix value"
